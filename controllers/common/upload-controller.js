...
export const addAttachments = async (req, res) => {
  try {
    ...
    // Vérifier si l'utilisateur est autorisé (admin ou le stagiaire propriétaire)
    if (req.user.role !== "admin" && reclamation.stagiaire.toString() !== req.user._id.toString()) {
      // Supprimer les fichiers téléchargés
      req.files.forEach((file) => {
        fs.unlinkSync(file.path)
      })

      return res.status(403).json({ message: "Non autorisé à modifier cette réclamation" })
    }
